...
event3duration = 2;    % duration of feedback
...
ISImean = 6;            % For 'exponential' only.  Includes ISImin.  There is an optimal empirical value -- longer is better for deconvolution/FIR, but we also need to fit within total scan time constraints.
...
ISImax = 8;             % Truncate to avoid VERY long ISIs
...
eventlist = [onsets, onsets + event1duration + finalISI1, onsets + event1duration + finalISI1 + event2duration + finalISI2, trialtype, finalISI1, finalISI2, finalISI3];
...
design_struct.eventlist_names = {'Event1 onset' 'Event2 onset' 'Event3 onset' 'Trial type' 'ISI1' 'ITI' 'Event1Dur' 'Event2Dur' 'Event3Dur'};
