function [meanrecipvif, vifs, design_struct] = generate_jittered_er_design_ENGRAMS_RCG(varargin)
...
event1duration = 2;    % duration of scene cue
event2duration = 2.5;    % duration of objects options
event3duration = 2.5;    % duration of confidence judgement
event4duration = 2;    % duration of answer

trialtypes = 1;        % neutral, 2 levels of loss, 2 levels of gain
trialspertype = 60;
...
ISImean = 3;            % For 'exponential' only.  Includes ISImin.  There is an optimal empirical value -- longer is better for deconvolution/FIR, but we also need to fit within total scan time constraints.
...
switch isidistribution
    ...
    case 'exponential'
        ...
        if ~ISI2constantvalue  % variable ITI/ISI2
            ...
            ISI3 = ISImin + exprnd(ISImean - ISImin, ntrials, 1);
            ISI3(ISI3 > ISImax) = ISImax;
            
            ISI4 = ISImin + exprnd(ISImean - ISImin, ntrials, 1);
            ISI4(ISI4 > ISImax) = ISImax;
...
for i = 1:trialtypes
    ...
    if ISI2isconstant
        ...
        finalISI3{i} = ISI2constantvalue * ones(size(finalISI3{i}));
        finalISI4{i} = ISI2constantvalue * ones(size(finalISI4{i}));
...
for i = 1:trialtypes
    ...
    ons3{i} = ons{i} + event1duration + finalISI1(trialtype == i) + event2duration + finalISI2(trialtype == i);

    ons4{i} = ons{i} + event1duration + finalISI1(trialtype == i) + event2duration + finalISI2(trialtype == i) + event3duration + finalISI3(trialtype == i);
    
end

ons = [ons ons2 ons3 ons4];
...
design_struct = struct('ons', [], 'TR', TR, 'X', X, 'event1duration', event1duration, 'event2duration', event2duration, 'event3duration', event3duration, 'event4duration', event4duration, 'trialtypes', trialtypes, 'trialspertype', trialspertype, 'scanlength', ceil(scanlength));
...
design_struct.eventlist_names = {'Event1 onset' 'Event2 onset' 'Event3 onset' 'Event4 onset' 'Trial type' 'ISI1' 'ISI2' 'ISI3' 'ITI' 'Event1Dur' 'Event2Dur' 'Event3Dur' 'Event4Dur'};
