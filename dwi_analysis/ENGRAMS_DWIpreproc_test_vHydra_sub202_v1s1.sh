#!/bin/bash
#$ -S /bin/bash
#$ -l h_rt=168:00:00,h_vmem=12G,mem_free=12G
#$ -q work.q
...
SUBJECT="sub-202v1s1" # to do: sub-102v2s1, sub-102v2s2, sub-105v1s2, sub-107v1s1, sub-107v1s2, sub-107v2s1, sub-107v2s2

# places
DATA_DIR="/mnt/work/yyi/ENGRAMS/preproc/${SUBJECT}/dwi"
...
/share/apps/freesurfer_7.3/bin/mri_synthstrip -i $OUTPUT_DIR/${SUBJECT}_hifi_b0.nii.gz -o $OUTPUT_DIR/${SUBJECT}_nodif_hifi_b0_brain.nii.gz -m $OUTPUT_DIR/${SUBJECT}_nodif_hifi_b0_brain_mask.nii.gz
...
echo "create index file for eddt based on merged dwi time-series components"
...
for file in "${dwi_files[@]}"; do
    nvols=$(fslval "$file" dim4 | tr -d '[:space:]')  # clean up any spaces or newlines
    if ! [[ "$nvols" =~ ^[0-9]+$ ]]; then
        echo "error: volume count for $file is invalid: '$nvols'" >&2
        exit 1
    fi

    [[ "$file" == *-AP_dwi.nii.gz ]] && index="1" || index="2"  # AP=1, PA=2
