...
for i = 1:length(acquisitions)
    ...
    % otsu mask (but maybe try our own bin mask)
    brain_mask = orig_b0 > (0.5 * max(orig_b0(:)));
    brain_mask = bwareaopen(brain_mask, 1000);
    
    % mask
    [X, Y, Z] = size(orig_b0);
    background_mask = false(size(orig_b0));
    background_mask([1:10, end-9:end], [1:10, end-9:end], :) = true;
    background_mask = background_mask & (orig_b0 < 100);  % Only low intensity
    
    % snr
    orig_signal = mean(orig_b0(brain_mask));
    orig_noise = std(orig_b0(background_mask));
    orig_snr = orig_signal / orig_noise;
    
    nordic_signal = mean(nordic_b0(brain_mask));
    nordic_noise = std(nordic_b0(background_mask));
    nordic_snr = nordic_signal / nordic_noise;
    
    snr_improvement = ((nordic_snr - orig_snr) / orig_snr) * 100;
    
    fprintf('\n%s:\n', acq);
    fprintf('  Brain signal:        %.1f -> %.1f\n', orig_signal, nordic_signal);
    fprintf('  Background noise:    %.2f -> %.2f\n', orig_noise, nordic_noise);
    fprintf('  SNR:                 %.1f -> %.1f (%.1f%% improvement)\n', ...
        orig_snr, nordic_snr, snr_improvement);
end

%% look at gfactor too

mat_file = fullfile(out_dir, 'sub-302v1s2_b1pt0k-AP_nordic.mat');
load(mat_file, 'ARG2');
