...
for id=1:length(ids)
    ...
    try
        eval(['!gzip -f -d ' paths_source ids{id} 'v1s1/func/' ids{id} 'v1s1_task-origrec_run-*_topup.nii.gz'])
        tmp=dir([paths_source ids{id} 'v1s1/func/' ids{id} 'v1s1_task-origrec_run-*_topup.nii']);
