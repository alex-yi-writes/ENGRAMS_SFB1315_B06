function [meanrecipvif, vifs, design_struct] = generate_jittered_er_design_ENGRAMS_ENC_behavpilot(varargin)
...
event1duration = 1.5;    % duration of scene
event2duration = 1.5;    % duration of object
event3duration = 1.5;    % duration of both presentation
trialtypes = 1;        % only one condition at a time
trialspertype = 60;
...
ISImax = 6.5;             % Truncate to avoid VERY long ISIs
...
for i = 1:trialtypes
    ...
    trialduration{i} = event1duration + finalISI1{i} + event2duration + finalISI2{i} + event3duration + finalISI3{i};
...
design_struct = struct('ons', [], 'TR', TR, 'X', X, 'event1duration', event1duration, 'event2duration', event2duration, 'event3duration', event3duration, 'trialtypes', trialtypes, 'trialspertype', trialspertype, 'scanlength', ceil(scanlength));
...
eventlist = [onsets,...
    ...
    trialtype, finalISI1, finalISI2, finalISI3];
...
design_struct.eventlist_names = {'Event1 onset' 'Event2 onset' 'Event3 onset' 'Trial type' 'ISI1' 'ISI2' 'ITI' 'Event1Dur' 'Event2Dur' 'Event3Dur'};
