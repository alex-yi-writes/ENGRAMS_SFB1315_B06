...
for id=1%:length(ids)
    ...
    for t1=1:numel(tasks)
        ...
        save(['/Users/alex/Dropbox/paperwriting/1315/data/segmentation/' ...
            subj 'v1s1/analyses/all/Z_' subj '_origrec.mat'], 'Z','R')
...
for id=1%:length(ids)
    ...
    for t1=1:numel(tasks)
        ...
        axis off equal
        title(['' subj ' : |pearson''s r| > 0.2  (red=pos, blue=neg)'],'Fontsize',20,'FontWeight','bold')
        ...
        colorbar; title([subj '  ROI connectivity (Fisher-z)'])
