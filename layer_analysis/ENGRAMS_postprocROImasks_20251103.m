...
clc;clear
ids     = {'sub-301'}%{'sub-202','sub-207','sub-302','sub-303','sub-304'};%{'sub-101','sub-104','sub-106','sub-107','sub-108','sub-109','sub-110'};%{'sub-302','sub-303','sub-304'}; % do 'sub-106' later
tasks_v1   = {'rest','origenc','origrec1'};
tasks_v2   = {'origrec2','recombienc','recombirec1'};
...
for id=1:length(ids)
    ...
    for t1=1:numel(tasks_v1)
        ...
        % postproc_rois   = {'EC_layer_deep','EC_layer_mid','EC_layer_sup'};%{'mPFC_layer_deep','mPFC_layer_mid','mPFC_layer_sup','CA1','CA3','DG','Sub','ErC',...
...
for id=1:length(ids)
    ...
    for t1=1:numel(tasks_v2)
        ...
        % CSF
        clear matlabbatch tmp
        tmp=dir([ path_par subj 'v2s1/func/mean' subj '*_task-' tasks_v2{t1} '_run-0*_bold_topup_stripped.nii']);
        if isempty(tmp)
        tmp=dir([ path_par subj 'v2s1/func/mean' subj '*_task-' tasks_v2{t1} '1_run-0*_bold_topup_stripped.nii.gz']);
        eval(['!gzip -d ' fullfile(tmp.folder, tmp.name)])
        meanepi=[tmp.folder '/' tmp.name(1:end-3)];
        else
        meanepi=[tmp.folder '/' tmp.name];
        ...
        matlabbatch{1}.spm.spatial.preproc.warp.bb = [NaN NaN NaN
            NaN NaN NaN];
        matlabbatch{2}.spm.util.imcalc.input(1) = cfg_dep('Segment: c3 Images', substruct('.','val', '{}',{1}, '.','val', '{}',{1}, '.','val', '{}',{1}), substruct('.','tiss', '()',{2}, '.','c', '()',{':'}));
        matlabbatch{2}.spm.util.imcalc.output = [tasks_v2{t1} '_csf_bin'];
        matlabbatch{2}.spm.util.imcalc.outdir = {[tmp.folder]};
        matlabbatch{2}.spm.util.imcalc.expression = 'i1>0.6';
        matlabbatch{2}.spm.util.imcalc.var = struct('name', {}, 'value', {});
        matlabbatch{2}.spm.util.imcalc.options.dmtx = 0;
