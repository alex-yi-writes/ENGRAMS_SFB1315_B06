...
%     flags{id,1}.origrec2.SOcorr=1;
    ...
    clear fMRI nvols tmp
    eval(['!gzip -f -d ' paths_source ids{id} 'v2s1/func/' ids{id} 'v2s1_task-recombirec_run-*_topup.nii.gz'])
    tmp=dir([paths_source ids{id} 'v2s1/func/' ids{id} 'v2s1_task-recombirec_run-*_topup.nii']);
...
%         eval(['!rm ' paths_source ids{id} 'v1s1/func/' tmp(t1).name])
    ...
    mkdir(['/Volumes/IKND/AG_Haemmerer/Alex-Qumulo/ENGRAMS/preproc/' ids{id} 'v2s1/func/'])

    clear tmp
    tmp=dir([paths_source ids{id} 'v2s1/func/rsub*topup*.nii*']);
    for t1=1:length(tmp)
        copyfile([paths_source ids{id} 'v2s1/func/' tmp(t1).name],...
            ['/Volumes/IKND/AG_Haemmerer/Alex-Qumulo/ENGRAMS/preproc/' ids{id} 'v2s1/func/' tmp(t1).name])
%         eval(['!rm ' paths_source ids{id} 'v1s1/func/' tmp(t1).name])
    end

    clear tmp
    tmp=dir([paths_source ids{id} 'v2s1/func/arsub*topup*.nii*']);
    for t1=1:length(tmp)
        copyfile([paths_source ids{id} 'v2s1/func/' tmp(t1).name],...
            ['/Volumes/IKND/AG_Haemmerer/Alex-Qumulo/ENGRAMS/preproc/' ids{id} 'v2s1/func/' tmp(t1).name])
%         eval(['!rm ' paths_source ids{id} 'v1s1/func/' tmp(t1).name])
    ...
    clear tmp
    tmp=dir([paths_source ids{id} 'v2s1/func/sub*topup*.nii*']);
    for t1=1:length(tmp)
        copyfile([paths_source ids{id} 'v2s1/func/' tmp(t1).name],...
            ['/Volumes/IKND/AG_Haemmerer/Alex-Qumulo/ENGRAMS/preproc/' ids{id} 'v2s1/func/' tmp(t1).name])
%         eval(['!rm ' paths_source ids{id} 'v1s1/func/' tmp(t1).name])
