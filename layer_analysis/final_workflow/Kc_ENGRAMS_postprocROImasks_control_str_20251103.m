...
clc;clear
ids         = {'sub-102'}%{'sub-202','sub-207','sub-302','sub-303','sub-304'};%{'sub-101','sub-104','sub-106','sub-107','sub-108','sub-109','sub-110'};%{'sub-302','sub-303','sub-304'}; % do 'sub-106' later
tasks_v1    = {'rest'};
tasks_v2    = {'origenc','origrec'};
...
for id=1:length(ids)
    ...
    for t1=1:numel(tasks_v2)
        ...
        if isempty(tmp)
        tmp=dir([ path_par subj 'v2s1/func/mean' subj '*_task-' tasks_v2{t1} '_run-0*_bold_topup_stripped.nii.gz']);
        eval(['!gzip -d ' fullfile(tmp.folder, tmp.name)])
        meanepi=[tmp.folder '/' tmp.name(1:end-3)];
        else
        meanepi=[tmp.folder '/' tmp.name];
