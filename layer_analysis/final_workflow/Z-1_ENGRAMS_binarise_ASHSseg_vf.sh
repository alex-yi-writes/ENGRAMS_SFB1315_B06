...
bases=(CA1 CA2 DG CA3 Tail Label6 Label7 Sub ErC A35 A36 PhC Cysts Label17)
for b in "${bases[@]}"; do
  Lmask="$OUTDIR/${b}_L.nii.gz"
  Rmask="$OUTDIR/${b}_R.nii.gz"
  outBilateral="$OUTDIR/${b}.nii.gz"
  echo "→ combining ${b}_L and ${b}_R to ${b}.nii.gz"
  fslmaths "$Lmask" -add "$Rmask" -bin "$outBilateral"
done
