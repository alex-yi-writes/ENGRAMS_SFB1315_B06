...
%% SAFE ZONE! CLICK BELOW BEFORE RUNNING THE TASK!
%
%
%
%
...
%         [dat] = engrams_encoding_pupil(id,block,condition,phase,mri,pupil,taskpath,prepulse)
        [dat] = engrams_encoding(id,block,condition,phase,mri,pupil,taskpath,prepulse)
    ...
    elseif strcmp(tasktype,'Recognition')==1
        ...
        [dat] = engrams_recognition(id,block,condition,phase,mri,pupil,taskpath,prepulse)
        
        % simplify the accuracy result
        clear tmp acc
        tmp=dat.rcg.results.accuracy;
        acc=tmp==1;
        
        if nanmean(dat.rcg.results.accuracy) >= 0.8
            message = sprintf('Accuracy: %.2f\nTraining completed', nanmean(acc));
            f = msgbox(message);
        elseif nanmean(dat.rcg.results.accuracy) < 0.8
            message = sprintf('Accuracy: %3.2f\nRun another block to achieve over 80%% accuracy!', nanmean(acc));
            f = msgbox(message,'Warning','error');
        end
